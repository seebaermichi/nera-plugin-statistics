...
executing it — the hook against real YAML, the publish script in a temp project,
and the templates rendered with pug.

### Fixed

-   **mapping values no longer produce an `[object Object]` row.**
    `author: {name: …, url: …}` is an ordinary frontmatter shape, and
    stringifying it put user-visible garbage into the rendered table rather than
    failing the build. Mappings and nested arrays are now ignored, like `null`
    and empty strings. `Date` values are deliberately still counted: an unquoted
    `date: 2024-05-01` is parsed into a Date by the YAML loader, so dropping
    every non-primitive would have silently stopped counting a field that counts
    today
-   the README claimed publishing "skips templates that already exist". The
    check is on the **directory** — if `views/vendor/plugin-statistics/` exists,
    the command copies nothing and still exits 0, even when a file inside it is
    missing. Documented accurately, including the fact that **upgrading never
    updates published templates** without `--force`
-   **Node.js** in Compatibility said `>= 18`; the real floor has been
    `>=20.0.0` since 2.2.0
-   the Styling section omitted `.statistics--cards` and `.statistics--list`,
    the two block modifiers that distinguish the card and list layouts from the
    table layout

### Added

-   Configuration now states that `config/statistics.yaml` is **required**, that
    `count` must be a YAML list (`count: category` is silently rejected), and
    what a missing or invalid `count` produces
-   a note that `sort_by: count` inherits `sort_order: asc` and therefore lists
    the *least* used values first — a most-popular-first list needs both keys
-   documentation that counting is case-sensitive while sorting is not, so
    `Tech` and `tech` split into two adjacent entries
-   documentation that counts include pages with no `layout`, which the
    generator never renders, and exclude pages generated by other plugins such
    as `@nera-static/plugin-tags` — with the `config/plugin-order.yaml` opt-in
    for sites that want them
-   `## 📊 Generated Output` now shows the real markup of all three templates,
    generated from the shipped `.pug` files and diffed against them, instead of
    a sentence restating `app.statistics`
-   `## 🤝 Contributing`, linking to the Nera contributing guide
-   `**Plugin Utils**: ^1.2.0` in Compatibility, and the reason the floor is
    load-bearing
-   a statement that the BEM class names are a public contract, so renaming one
    is a major bump

### Changed

-   Template Publishing is its own `## 🛠️` section instead of a `###` buried
    under Usage
-   the Usage example includes **one** template rather than all three stacked,
    which rendered the same statistics three times over
...
-   the Nera compatibility line names v4.1.0 as a baseline rather than an
    unexplained requirement, and points out that the optional plugin-order
    technique needs v4.2.0+ and the root-absolute include form v4.3.0+

### Migration from v2.2.0

Nothing to do for a correctly configured site, and no template markup changed —
you do **not** need to re-publish templates for this release.

The one behaviour change: a frontmatter property whose value is a **mapping** or
a **nested list** is no longer counted. Previously each one added an entry named
`[object Object]`. If your rendered statistics contained such a row, it now
disappears; nothing that was displaying a real value changes. Counting a mapping
was never useful, so if you want those pages counted, add a scalar field
(`author_name: Ada`) and count that instead.
